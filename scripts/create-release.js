...
function validateVersion(version) {
  if (!SEMVER_REGEX.test(version)) {
    throw new Error(`Invalid version format: ${version}. Must follow semantic versioning (e.g., 1.0.0, 1.0.0-beta.1)`);
  }
  return true;
}
...
function checkGitClean() {
  try {
    ...
    if (status) {
      throw new Error('Git working directory is not clean. Please commit or stash changes before creating a release.');
    }
  } catch (error) {
...
function runTests() {
  ...
  try {
    ...
    throw new Error('Tests failed. Please fix tests before creating a release.');
  }
}
...
function generateChangelog(version, previousTag) {
  try {
    let commits;
    if (previousTag) {
      commits = execSync(`git log ${previousTag}..HEAD --pretty=format:"- %s (%h)"`, { encoding: 'utf-8' }).trim();
    } else {
      commits = execSync('git log --pretty=format:"- %s (%h)"', { encoding: 'utf-8' }).trim();
    }
...
function updateChangelog(version) {
  ...
  try {
    previousTag = execSync('git describe --tags --abbrev=0 HEAD^ 2>/dev/null', { encoding: 'utf-8' }).trim();
  } catch {
...
function createGitTag(version) {
  ...
  if (tagExists(tag)) {
    throw new Error(`Tag ${tag} already exists. Please use a different version.`);
  }
...
function main() {
  ...
  try {
    ...
    if (versionArg) {
      ...
      throw new Error('Version argument required. Usage: node create-release.js <version> [--skip-tests] [--skip-changelog] [--dry-run]');
    }
    ...
    if (!dryRun) {
      ...
      if (!skipChangelog && fs.existsSync(path.join(__dirname, '../CHANGELOG.md'))) {
      ...
      console.log(`4. GitHub Actions will automatically build and create release`);
    } else {
