...
function validateVersion(version) {
  if (!SEMVER_REGEX.test(version)) {
    errors.push(`Invalid version format: ${version}. Must follow semantic versioning (e.g., 1.0.0)`);
    return false;
  }
  return true;
}
...
function validateDebFile(filePath) {
  try {
    ...
    if (!header.startsWith('!<arch>')) {
      warnings.push(`File ${path.basename(filePath)} may not be a valid .deb file`);
    }
...
function checkGitTag(version) {
  ...
  try {
    ...
    warnings.push(`Git tag ${tag} does not exist. Release may not be properly tagged.`);
    return false;
  }
}
...
function validateReleaseNotes() {
  ...
  if (fs.existsSync(changelogPath)) {
    ...
    warnings.push('CHANGELOG.md does not exist. Consider adding release notes.');
    return false;
  }
}
...
function checkElectronUpdaterConfig() {
  try {
    ...
    if (buildConfig.publish.provider !== 'github') {
      warnings.push(`Publish provider is ${buildConfig.publish.provider}, expected 'github'`);
    }
    
    if (!buildConfig.publish.owner || !buildConfig.publish.repo) {
      errors.push('GitHub owner and repo must be configured in publish settings');
      return false;
    }
