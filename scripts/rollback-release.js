...
function deprecateRelease(version, reason) {
  ...
  console.log(`1. Go to: https://github.com/masjidSolutions/masjidconnect-display-app/releases/tag/${tag}`);
  console.log('2. Click "Edit release"');
  console.log('3. Check "Set as a pre-release" to hide it from auto-updater');
  console.log(`4. Add deprecation notice: ${reason || 'This release has been deprecated due to issues'}`);
...
function createPatchRelease(rollbackVersion, reason) {
  ...
  console.log(`2. Commit: git add package.json && git commit -m "Rollback: Create patch release ${newVersion} to replace ${rollbackVersion}"`);
  console.log(`3. Tag: git tag -a v${newVersion} -m "Rollback release ${newVersion} - replaces ${rollbackVersion}"`);
...
function main() {
  ...
  try {
    if (!versionArg) {
      throw new Error('Version argument required. Usage: node rollback-release.js <version> [--create-patch] [--reason="reason"]');
    }
    ...
    if (createPatch) {
      ...
      console.log('   Use --create-patch flag to automatically create a replacement release');
    }
