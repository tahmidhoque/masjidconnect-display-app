...
const addCacheControlHeaders = (maxAge, staleWhileRevalidate) => {
  return (req, res, next) => {
    res.set('Cache-Control', `max-age=${maxAge}, stale-while-revalidate=${staleWhileRevalidate}`);
    next();
  };
};
...
app.get('/api/prayer-times', addCacheControlHeaders(86400, 604800), async (req, res) => {
  try {
...
app.get('/api/prayer-status', addCacheControlHeaders(60, 3600), async (req, res) => {
  try {
    ...
    const now = new Date();
    const { currentPrayer, nextPrayer } = calculatePrayerStatus(prayerTimes, now);
    ...
    try {
      ...
      res.json = function(body) {
        if (body && body.success) {
          ...
          setexAsync(cacheKey, expiryInSeconds, JSON.stringify(body))
...
app.get('/api/prayer-times', cacheMiddleware('prayer-times', 86400), async (req, res) => {
