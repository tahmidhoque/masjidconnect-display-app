  ...
  useEffect(() => {
      ...
      try {
        ...
        if (now.isAfter(prayerDayjs) && jamaatTime) {
          ...
          if (isNaN(jamaatHours) || isNaN(jamaatMinutes)) {
            logger.error(
              `[PrayerCountdown] Invalid jamaat time values for ${prayerName}: ${jamaatTime}`
            );
            return;
          }
          ...
          if (now.isBefore(jamaatDayjs)) {
            ...
            logger.info(
              `[PrayerCountdown] Starting in jamaat countdown mode for ${prayerName}`
            );
          }
        }
      } catch (error) {
        logger.error("[PrayerCountdown] Error in initialization", {
          error,
          prayerName,
          prayerTime,
          jamaatTime,
    ...
    const calculateRemainingTime = () => {
      try {
        ...
        if (isNaN(prayerHours) || isNaN(prayerMinutes)) {
          logger.error(
            `[PrayerCountdown] Invalid prayer time parts for ${prayerName}: ${prayerTime}`
          );
          return;
        }
        ...
        if (countingDownToJamaat && jamaatTime) {
          ...
          try {
            ...
            if (isNaN(jamaatHours) || isNaN(jamaatMinutes)) {
              logger.error(
                `[PrayerCountdown] Invalid jamaat time for ${prayerName}: ${jamaatTime}`
              );
              return;
            }
            ...
            if (now.isAfter(targetDayjs)) {
              logger.debug(
                `[PrayerCountdown] Jamaat time passed for ${prayerName}, using tomorrow's prayer time`
              );
            ...
            logger.error(
              `[PrayerCountdown] Error processing jamaat time: ${errorMessage}`,
              {
                prayerName,
                jamaatTime,
          ...
          if (now.isAfter(prayerDayjs)) {
            ...
            if (!prayerTimePassedRef.current && jamaatTime) {
              ...
              logger.info(
                `[PrayerCountdown] Prayer time passed for ${prayerName}, checking jamaat countdown`
              );
              ...
              try {
                ...
                if (isNaN(jamaatHours) || isNaN(jamaatMinutes)) {
                  ...
                  if (now.isBefore(jamaatDayjs)) {
                    ...
                    if (
                      ...
                      logger.info(
                        `[CRITICAL] ${prayerName} jamaat time has just passed (${jamaatTime})`
                      );
                ...
                logger.error(
                  `[PrayerCountdown] Error handling prayer-to-jamaat transition: ${errorMessage}`,
                  {
                    prayerName,
                    prayerTime,
                    jamaatTime,
        ...
        if (newSeconds === 0 && newMinutes % 1 === 0) {
          // Only log every minute, not every second
          logger.debug(
            `[PrayerCountdown] ${prayerName} countdown: ${formatRemainingTimeLog(
              newHours,
              newMinutes,
              newSeconds
            )}`,
        ...
        logger.error("[PrayerCountdown] Error calculating remaining time", {
          error,
        });
      }
    };
    ...
    if (onCountdownComplete) {
      try {
        ...
        setTimeout(() => {
          if (onCountdownComplete) {
            logger.info(
              "[PrayerCountdown] Calling onCountdownComplete callback",
              {
                isForJamaat,
                prayerName,
              }
        ...
        logger.error("[PrayerCountdown] Error in countdown complete callback", {
          error,
        });
      }
    }
