...
interface CrashReport {
  timestamp: string;
  type: 'javascript_error' | 'promise_rejection' | 'component_error' | 'network_error';
  error: string;
  stack?: string;
...
class CrashLogger {
  ...
  private setupPerformanceMonitoring(): void {
    ...
    if ('PerformanceObserver' in window) {
      try {
        ...
        logger.debug('PerformanceObserver not supported for longtask', { error });
      }
    }
  }
  ...
  public logCrash(crash: Partial<CrashReport>): void {
    ...
    try {
      const existingCrashes = JSON.parse(localStorage.getItem('masjidconnect_crashes') || '[]');
      existingCrashes.push(fullCrash);
      
      // Keep only last 20 in localStorage
      const crashesToSave = existingCrashes.slice(-20);
      localStorage.setItem('masjidconnect_crashes', JSON.stringify(crashesToSave));
    } catch (error) {
      logger.debug('Failed to save crash to localStorage', { error });
    }
  }

  public logComponentError(error: Error, errorInfo: { componentStack: string }): void {
    this.logCrash({
  ...
  public generateCrashReport(): string {
    ...
    const crashesByType = allCrashes.reduce((acc, crash) => {
