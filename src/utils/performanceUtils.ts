...
export const isLowPowerDevice = (): boolean => {
  ...
  if (window.performance && (window.performance as any).memory) {
    ...
    if (memoryInfo.jsHeapSizeLimit && memoryInfo.jsHeapSizeLimit < 1073741824) {
      // Less than 1GB
      return true;
    }
  }
...
export const getDevicePerformanceProfile = () => {
  ...
  if (isHighStrain) {
    ...
    profile = "ultra-low";
  } else if (isLowPower || cores <= 2 || memory < 536870912) {
    // Less than 512MB
    profile = "low";
  } else if (cores <= 4 || memory < 1073741824) {
    // Less than 1GB
    profile = "medium";
  }
...
export const optimizeAppPerformance = (): void => {
  ...
  document.documentElement.style.setProperty(
    "--transition-duration",
    `${recommendations.animationDuration}ms`,
  );
  document.documentElement.style.setProperty(
    "--animation-duration",
    `${recommendations.animationDuration * 2}ms`,
  );
  document.documentElement.style.setProperty(
    "--debounce-delay",
    `${recommendations.debounceDelay}ms`,
  );
...
class RPiMemoryManager {
  ...
  public startMonitoring(): void {
    ...
    if (process.env.NODE_ENV === "development") {
      console.log(
        "[RPiMemoryManager] Skipping memory monitoring in development mode",
      );
      return;
    }
  ...
  private performEmergencyCleanup(): void {
    ...
    console.warn(
      "[RPiMemoryManager] High memory usage detected - performed gentle cleanup only",
    );
  }
