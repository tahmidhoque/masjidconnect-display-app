...
export const testCorsForEndpoint = async (
  endpoint,
  apiKey = null,
  screenId = null,
) => {
...
export const testAllEndpoints = async (apiKey = null, screenId = null) => {
  ...
  console.log(
    `API URL: ${process.env.REACT_APP_API_URL || "http://localhost:3000"}`,
  );
  console.log(
    `Auth: ${apiKey ? "Yes" : "No"}, Screen ID: ${screenId ? "Yes" : "No"}`,
  );
  ...
  console.log(
    `===== COMPLETED: ${successCount}/${endpoints.length} endpoints passed =====`,
  );
...
if (typeof window !== "undefined") {
  ...
  console.log(
    "CORS testing utilities loaded. Use window.corsTest.testAllEndpoints() in the console to test.",
  );
}
