  ...
  return new Promise((resolve) => {
    ...
    const testTimer = setInterval(() => {
      ...
      if (interval > 1100) {
        skippedCount++;
        console.warn(
          `⚠️ Potential skipped second detected: ${interval}ms interval`,
        );
      }
...
export const testTimerInterference = (
  ...
  console.log(
    `🔄 Testing ${timerCount} concurrent timers for ${durationSeconds} seconds...`,
  );
 ...
 */
export const generateCountdownReport = (
  results: CountdownTestResult[],
): void => {
