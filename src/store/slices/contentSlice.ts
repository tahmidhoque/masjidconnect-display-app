...
export const refreshSchedule = createAsyncThunk(
  ...
  async ({ forceRefresh = false }: { forceRefresh?: boolean } = {}, { rejectWithValue, getState }) => {
    try {
      ...
      const state = getState() as { content: ContentState };
      const now = Date.now();
      const lastUpdate = state.content.lastScheduleUpdate ? new Date(state.content.lastScheduleUpdate).getTime() : 0;
      
      // Check if we should skip this refresh due to rate limiting
      if (!forceRefresh && now - lastUpdate < MIN_REFRESH_INTERVAL) {
        logger.debug('[Content] Skipping schedule refresh due to rate limiting');
        return { skipped: true };
      }
...
export const refreshAllContent = createAsyncThunk(
  ...
  async ({ forceRefresh = false }: { forceRefresh?: boolean } = {}, { dispatch }) => {
    try {
      ...
      results.forEach((result, index) => {
        if (result.status === 'rejected') {
          ...
          logger.warn(`[Content] Failed to refresh ${names[index]}`, { reason: result.reason });
        }
      });
...
const contentSlice = createSlice({
  ...
  reducers: {
    ...
    setPrayerAnnouncement: (state, action: PayloadAction<{
      show: boolean;
      prayerName?: string;
      isJamaat?: boolean;
    }>) => {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(refreshSchedule.fulfilled, (state, action) => {
        ...
        // Update general loading state
        state.isLoading = state.isLoadingContent || state.isLoadingPrayerTimes || state.isLoadingEvents;
      })
    ...
    builder
      ...
      .addCase(refreshEvents.fulfilled, (state, action) => {
        ...
        // Update general loading state
        state.isLoading = state.isLoadingContent || state.isLoadingPrayerTimes || state.isLoadingSchedule;
      })
...
// Selectors
export const selectIsLoading = (state: { content: ContentState }) => state.content.isLoading;
export const selectScreenContent = (state: { content: ContentState }) => state.content.screenContent;
export const selectPrayerTimes = (state: { content: ContentState }) => state.content.prayerTimes;
export const selectSchedule = (state: { content: ContentState }) => state.content.schedule;
export const selectEvents = (state: { content: ContentState }) => state.content.events;
export const selectMasjidName = (state: { content: ContentState }) => state.content.masjidName;
export const selectMasjidTimezone = (state: { content: ContentState }) => state.content.masjidTimezone;
export const selectCarouselTime = (state: { content: ContentState }) => state.content.carouselTime;
export const selectShowPrayerAnnouncement = (state: { content: ContentState }) => state.content.showPrayerAnnouncement;
export const selectPrayerAnnouncementName = (state: { content: ContentState }) => state.content.prayerAnnouncementName;
export const selectIsPrayerJamaat = (state: { content: ContentState }) => state.content.isPrayerJamaat;
export const selectLastUpdated = (state: { content: ContentState }) => state.content.lastUpdated;
export const selectContentError = (state: { content: ContentState }) => state.content.contentError;
export const selectPrayerTimesError = (state: { content: ContentState }) => state.content.prayerTimesError;
export const selectScheduleError = (state: { content: ContentState }) => state.content.scheduleError;
export const selectEventsError = (state: { content: ContentState }) => state.content.eventsError;

// Loading selectors
export const selectIsLoadingContent = (state: { content: ContentState }) => state.content.isLoadingContent;
export const selectIsLoadingPrayerTimes = (state: { content: ContentState }) => state.content.isLoadingPrayerTimes;
export const selectIsLoadingSchedule = (state: { content: ContentState }) => state.content.isLoadingSchedule;
export const selectIsLoadingEvents = (state: { content: ContentState }) => state.content.isLoadingEvents;
