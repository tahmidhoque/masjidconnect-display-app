...
function getUserFriendlyMessage(code: ErrorCode, originalMessage: string): string {
  const messages: Record<ErrorCode, string> = {
    [ErrorCode.NET_OFFLINE]: 'Your device appears to be offline. Please check your internet connection.',
    [ErrorCode.NET_TIMEOUT]: 'The connection is taking longer than usual. Please wait or try again.',
    [ErrorCode.NET_CONNECTION_FAILED]: 'Unable to connect to the server. Please check your network settings.',
    [ErrorCode.NET_DNS_FAILED]: 'Cannot resolve server address. Please check your DNS settings.',
    [ErrorCode.NET_CORS_BLOCKED]: 'Server configuration issue. Please contact your system administrator.',
    
    [ErrorCode.AUTH_INVALID_TOKEN]: 'Authentication token is invalid. Please re-pair this device.',
    [ErrorCode.AUTH_TOKEN_EXPIRED]: 'Authentication has expired. Please re-pair this device.',
    [ErrorCode.AUTH_SCREEN_NOT_PAIRED]: 'This display is not yet paired with a masjid account.',
    [ErrorCode.AUTH_PAIRING_FAILED]: 'Failed to pair device. Please try again or contact support.',
    [ErrorCode.AUTH_API_KEY_INVALID]: 'API credentials are invalid. Please re-pair this device.',
    
    [ErrorCode.API_SERVER_DOWN]: 'The server is temporarily unavailable. Using cached data where possible.',
    [ErrorCode.API_RATE_LIMITED]: 'Too many requests. The system will automatically retry in a moment.',
    [ErrorCode.API_INVALID_RESPONSE]: 'Received unexpected data from server. Retrying...',
    [ErrorCode.API_ENDPOINT_NOT_FOUND]: 'Server endpoint not found. Please check your configuration.',
    [ErrorCode.API_INTERNAL_ERROR]: 'Server encountered an error. Using cached data where possible.',
    
    [ErrorCode.DATA_PRAYER_TIMES_MISSING]: 'Prayer times are not available. Retrying...',
    [ErrorCode.DATA_CONTENT_MISSING]: 'Display content is not available. Using cached content.',
    [ErrorCode.DATA_MASJID_INFO_MISSING]: 'Masjid information is not available. Using default settings.',
    [ErrorCode.DATA_CACHE_CORRUPTED]: 'Local data cache needs to be refreshed. Please wait...',
    [ErrorCode.DATA_SYNC_FAILED]: 'Failed to sync latest data. Using cached information.',
    
    [ErrorCode.SYS_STORAGE_FULL]: 'Device storage is full. Please free up space and restart.',
    [ErrorCode.SYS_MEMORY_EXCEEDED]: 'System memory is low. Optimizing performance...',
    [ErrorCode.SYS_RENDER_FAILED]: 'Display rendering error. Attempting to recover...',
    [ErrorCode.SYS_SERVICE_WORKER_FAILED]: 'Background services failed. Restarting...',
    [ErrorCode.SYS_ELECTRON_ERROR]: 'Application system error. Please restart the application.',
    
    [ErrorCode.APP_INITIALIZATION_FAILED]: 'Failed to initialize application. Please restart.',
    [ErrorCode.APP_COMPONENT_CRASHED]: 'A component crashed. Attempting to recover...',
    [ErrorCode.APP_CONFIGURATION_INVALID]: 'Application configuration is invalid. Please contact support.',
    [ErrorCode.APP_UPDATE_FAILED]: 'Application update failed. Continuing with current version.',
    [ErrorCode.APP_UNKNOWN_ERROR]: 'An unexpected error occurred. Please restart if the issue persists.'
  };
...
const errorSlice = createSlice({
  ...
  reducers: {
    // Report a new error
    reportError: (state, action: PayloadAction<{
      code: ErrorCode;
      message: string;
      severity: ErrorSeverity;
      source?: string;
      ...
      if (!state.currentDisplayedError && (severity === ErrorSeverity.HIGH || severity === ErrorSeverity.CRITICAL)) {
    ...
    startRecovery: (state) => {
      const now = Date.now();
      const lastAttempt = state.lastRecoveryAttempt ? new Date(state.lastRecoveryAttempt).getTime() : 0;
    ...
    completeRecovery: (state, action: PayloadAction<{ success: boolean; message?: string }>) => {
      ...
      if (action.payload.success) {
        ...
        state.activeErrors = state.activeErrors.filter(error => !error.isRecoverable);
        ...
        logger.warn('[ErrorManager] Recovery failed', { message: action.payload.message });
      }
    },
...
function updateSystemHealth(state: ErrorState, error: AppError) {
  ...
  if (category === ErrorCategory.NETWORK) {
    state.systemHealth.api = severity === ErrorSeverity.CRITICAL ? 'down' : 'degraded';
    ...
    if (error.code === ErrorCode.SYS_STORAGE_FULL) {
      ...
      state.systemHealth.memory = severity === ErrorSeverity.CRITICAL ? 'critical' : 'high';
    }
  }
  ...
  const hasCritical = healthScores.some(h => h === 'critical' || h === 'down' || h === 'full');
  const hasDegraded = healthScores.some(h => h === 'degraded' || h === 'high');
...
// Selectors
export const selectActiveErrors = (state: { errors: ErrorState }) => state.errors.activeErrors;
export const selectCurrentError = (state: { errors: ErrorState }) => state.errors.currentDisplayedError;
export const selectNetworkStatus = (state: { errors: ErrorState }) => state.errors.networkStatus;
export const selectSystemHealth = (state: { errors: ErrorState }) => state.errors.systemHealth;
export const selectIsRecovering = (state: { errors: ErrorState }) => state.errors.isRecovering;
export const selectShowErrorOverlay = (state: { errors: ErrorState }) => state.errors.showErrorOverlay; 