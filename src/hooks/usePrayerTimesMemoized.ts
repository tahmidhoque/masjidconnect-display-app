...
export const usePrayerTimesMemoized = (): PrayerTimesHook => {
  ...
  const processedPrayerTimes = useMemo(() => {
    ...
    try {
      ...
      const formatted: FormattedPrayerTime[] = PRAYER_NAMES.map((name) => {
        ...
        const jamaat = todayData[jamaatKey as keyof typeof todayData] as
          | string
          | undefined;
      ...
      if (!nextPrayer && formatted.length > 0) {
        ...
        if (fajr) {
          ...
          const tomorrowFajr = dayjs(`${currentDate} ${fajr.time}`).add(
            1,
            "day",
          );
  ...
  useEffect(() => {
    const fetchHijriDate = async () => {
      ...
      if (
        ...
        now - hijriDateCache.current.timestamp < 3600000
      ) {
        // 1 hour
      ...
      try {
        ...
        logger.warn("Failed to calculate Hijri date, using fallback", {
          error,
        });
  ...
  const { isJumuahToday, jumuahTime, jumuahDisplayTime, jumuahKhutbahTime } =
    useMemo(() => {
      const today = dayjs();
