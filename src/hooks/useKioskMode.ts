...
export const useKioskMode = () => {
  ...
  const refreshData = useCallback(async () => {
    try {
      ...
      if (timeSinceLastRefresh < 10 * 60 * 1000) {
        logger.info(
          "[KioskMode] Skipping refresh - last refresh was less than 10 minutes ago",
        );
        return;
      }
  ...
  const handleVisibilityChange = useCallback(() => {
    if (document.visibilityState === "visible") {
      logger.info(
        "[KioskMode] App became visible, scheduling conservative refresh...",
      );
    ...
    const backupInterval = setInterval(
      () => {
        ...
        if (timeSinceLastRefresh > twoHoursMs) {
          logger.info(
            "[KioskMode] Backup polling: No refresh in 2 hours, triggering refresh",
          );
          refreshData();
        }
      },
      60 * 60 * 1000,
    ); // Check every hour instead of every 10 minutes
