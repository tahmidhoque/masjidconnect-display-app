...
export const useFactoryReset = (): UseFactoryResetResult => {
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    ...
    if (isModifierPressed && isShiftPressed && isRKey) {
      ...
      logger.info('⌨️ Factory reset keyboard shortcut triggered: Ctrl+Shift+R');
  ...
  useEffect(() => {
    logger.info('🎹 Factory reset keyboard shortcut active: Ctrl+Shift+R (or Cmd+Shift+R on Mac)');
