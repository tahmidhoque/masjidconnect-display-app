...
export function useContentAnalytics(
  ...
  const startTracking = useCallback(() => {
    if (isTrackingRef.current) {
      logger.warn("Content tracking already started", {
        contentId,
        contentType,
      });
      return;
  ...
  const stopTracking = useCallback(
    async (viewComplete = true) => {
      ...
      const duration =
        new Date(endTime).getTime() - new Date(startTimeRef.current).getTime();
  ...
  const reportError = useCallback(
    async (errorMessage: string) => {
      try {
        ...
        logger.debug("Content error reported", {
          contentId,
          contentType,
          errorMessage,
        });
  ...
  const trackLoadTime = useCallback(
    (loadTime: number) => {
      ...
      logger.debug("Content load time tracked", {
        contentId,
        contentType,
        loadTime,
      });
 ...
 */
export function useSimpleContentTracking(
  contentId: string,
  contentType: string,
) {
