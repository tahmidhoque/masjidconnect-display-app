...
describe.skip("DataSyncService - SKIPPED: Axios ESM import issues", () => {
  ...
  beforeEach(() => {
    ...
    mockedClient.getScreenContent = jest
      .fn()
      .mockResolvedValue(createSuccessResponse(mockScreenContent));
    mockedClient.getPrayerTimes = jest
      .fn()
      .mockResolvedValue(createSuccessResponse(mockPrayerTimesArray));
    mockedClient.getEvents = jest
      .fn()
      .mockResolvedValue(createSuccessResponse(mockEventsResponse));
    mockedClient.sendHeartbeat = jest
      .fn()
  ...
  describe("Content Syncing", () => {
    ...
    it("should sync content successfully", async () => {
      ...
      expect(mockedClient.getScreenContent).toHaveBeenCalled();
      expect(mockedStorage.saveScreenContent).toHaveBeenCalledWith(
        mockScreenContent,
      );
    });
  ...
  describe("Prayer Times Syncing", () => {
    ...
    it("should sync prayer times successfully", async () => {
      ...
      expect(mockedClient.getPrayerTimes).toHaveBeenCalled();
      expect(mockedStorage.savePrayerTimes).toHaveBeenCalledWith(
        mockPrayerTimesArray,
      );
    });
