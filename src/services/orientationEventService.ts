...
class OrientationEventService {
  ...
  public initialize(baseURL: string): void {
    ...
    if (this.isInitializing) {
      logger.warn(
        "OrientationEventService: Already initializing, skipping duplicate call",
      );
      return;
    }
    ...
    logger.info(
      "OrientationEventService: Initializing with unified SSE service",
      { baseURL },
    );
    console.log(
      "🔄 OrientationEventService: Initializing with unified SSE service",
  ...
  private registerEventHandlers(): void {
    ...
    const unregisterPrimary = unifiedSSEService.addEventListener(
      EVENT_TYPES.PRIMARY,
      (event: MessageEvent) => {
        console.log(
          `🔄 OrientationEventService: ${EVENT_TYPES.PRIMARY} event received via unified SSE!`,
          event.data,
        );
    ...
    altNames.forEach((eventName) => {
      const unregister = unifiedSSEService.addEventListener(
        ...
        (event: MessageEvent) => {
          console.log(
            `🔄 OrientationEventService: ${eventName} event received via unified SSE!`,
            event.data,
          );
          this.handleOrientationEvent(event);
        },
    ...
    const unregisterMessage = unifiedSSEService.addEventListener(
      ...
      (event: MessageEvent) => {
        ...
        logger.debug(
          "OrientationEventService: Received default message event via unified SSE",
          {
        ...
        console.log(
          "🔄 OrientationEventService: Default message event received:",
          messageEvent.data,
        );
        ...
        try {
          ...
          if (
            data &&
            (data.orientation || data.type === "SCREEN_ORIENTATION")
          ) {
          ...
          logger.debug(
            "OrientationEventService: Message event is not an orientation event",
          );
        }
      },
    );
    ...
    logger.info(
      "OrientationEventService: All event handlers registered with unified SSE service",
    );
    console.log(
      "🔄 OrientationEventService: All event handlers registered successfully",
    );
  }
  ...
  private handleOrientationEvent = (event: MessageEvent): void => {
    ...
    console.log(
      "🔍 OrientationEventService: SCREEN_ORIENTATION event received:",
      {
        type: event.type,
        data:
    ...
    if (!connectionStatus.connected) {
      logger.warn(
        "OrientationEventService: Ignoring orientation event - unified SSE connection not ready",
        {
      ...
      console.warn(
        "🔍 OrientationEventService: Orientation event blocked - unified SSE connection not ready",
      );
      return;
    }
    ...
    try {
      ...
      // Log the parsed data
      console.log(
        "🔍 OrientationEventService: Parsed orientation data:",
        orientationData,
      );
      ...
      if (
        ...
        logger.error(
          "OrientationEventService: Invalid orientation data format",
          { orientationData },
        );
        console.error(
          "❌ OrientationEventService: Missing required fields in orientation data",
        );
        return;
      }
      ...
      if (
        ...
        console.error(
          `❌ OrientationEventService: Invalid orientation value: ${orientationData.orientation}`,
        );
        return;
      }
      ...
      if (this.currentScreenId && orientationData.id !== this.currentScreenId) {
        logger.debug(
          "OrientationEventService: Ignoring orientation update for different screen",
          {
        ...
        console.log(
          `⏭️ OrientationEventService: Ignoring orientation update for screen ${orientationData.id} (we are ${this.currentScreenId})`,
        );
        return;
      }
      ...
      if (
        ...
        this.lastOrientationUpdate.orientation ===
          orientationData.orientation &&
        now - this.lastOrientationUpdate.timestamp <
          this.orientationUpdateDebounceTime
      ) {
        console.log(
          `⏭️ OrientationEventService: Debouncing orientation update to ${orientationData.orientation} (last update was ${(now - this.lastOrientationUpdate.timestamp) / 1000}s ago)`,
        );
        return;
      }
      ...
      console.log(
        `✅ OrientationEventService: Changing orientation from ${previousOrientation || "unknown"} to ${orientationData.orientation} for screen ${orientationData.id}`,
      );
      logger.info("OrientationEventService: Orientation change", {
        ...
        timestamp: now,
        transitionTime: previousOrientation
          ? now - (this.lastOrientationUpdate?.timestamp || now)
          : 0,
      });
      ...
      try {
        ...
        logger.debug(
          "OrientationEventService: Stored orientation in localStorage",
          {
            orientation: orientationData.orientation,
          },
        );
        ...
        console.error(
          "❌ OrientationEventService: Error storing orientation:",
          error,
        );
      }
      ...
      try {
        ...
        logger.debug(
          "OrientationEventService: Dispatched orientation-changed custom event",
        );
        console.log(
          "📢 OrientationEventService: Dispatched orientation-changed event",
        );
      } catch (error) {
        logger.error(
          "OrientationEventService: Error dispatching custom event",
          { error },
        );
        console.error(
          "❌ OrientationEventService: Error dispatching custom event:",
      ...
      logger.error(
        "OrientationEventService: Error processing orientation event",
        { error },
      );
      console.error(
        "❌ OrientationEventService: Error processing event:",
        error,
      );
      if (typeof event.data === "string") {
